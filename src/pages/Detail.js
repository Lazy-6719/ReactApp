...
export default class Detail extends Component{
  ...
  constructor(props){
    ...
    var apiname = qs.parse(props.location.search).apiname;
  ...
  renderPage=({title,des,time,detail:{auth,auth_icon,content}})=>(
    <div>
      ...
      <div className="content">
        <div className="header clear"><h2>
			<img width="50" src={`${this.baseUrl}/${auth_icon}`} alt=""/></h2><p>{auth}</p></div>
        <div className="cont">
          <h3>{title}</h3>
  ...
  BannerDetail=()=>(<BannerDetail data={this.state.data}/>);
  
  render(){
    let el=null;
    let apiname = qs.parse(this.props.location.search).apiname;
    if (this.state.data.title) {
      if(apiname === 'banner'){
        el = this.BannerDetail(this.state.data)
      }else{
